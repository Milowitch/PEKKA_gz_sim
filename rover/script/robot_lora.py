...
class Robot_LoRa(Node):
    def __init__(self):
        ...
        self.declare_parameter('serial_port', '/dev/lora0')  # String parameter
        self.declare_parameter('baud_rate', 115200)          # Integer parameter
        self.declare_parameter('send_rate', 2.0)            # Double parameter
        ...
        self.robot_lora_send = self.create_subscription(
            String,
            'robot_lora/send',
            self.robot_lora_send_callback,
            qos_profile)
        self.robot_lora_send
        self.last_robot_lora_send_callback_time = self.get_clock().now()

        self.publisher_ = self.create_publisher(String, 'robot_lora/receive', qos_profile)
        self.data = String()
    ...
    def robot_lora_send_callback(self, msg):
        try:
    ...
    def write_to_serial(self):
        while rclpy.ok():
            try:
                if self.send_data != '':
                    ...
                    if self.serial_port is not None and self.serial_port.is_open:
                        ...
                        size2 = sys.getsizeof(encoded_data + b'\r\n')
            ...
            if ((now - self.last_robot_lora_send_callback_time).nanoseconds / 1e9) >= self.lost_connect_time: # Convert to seconds
                self.send_data = ''
...
# SERIAL_PORT = '/dev/lora1'  # For Linux or MacOS
# BAUD_RATE = 115200