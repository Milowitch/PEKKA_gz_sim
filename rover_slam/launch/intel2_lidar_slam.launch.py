...
def generate_launch_description():
    ...
    declare_file = DeclareLaunchArgument(
        'file', default_value="'~/maps/mymap_2.db'",
        description='~/maps/mymap_2.db')
    ...
    rtabmap_util = Node(
        ...
        parameters=[{
            'max_clouds': 10,                # จำนวน point clouds ที่นำมารวมกัน
            'fixed_frame_id': 'lidar_link', # ไอดีเฟรมของ Lidar
            'use_sim_time': use_sim_time,   # ใช้ในการจำลอง
            'range_min': 0.25,              # ระยะ Lidar ใกล้สุด
            'range_max': 100.0,              # ระยะ Lidar ไกลสุด
        }],
        remappings=[
    ...
    parameters={
        'frame_id': 'base_footprint',   # ไอดีเฟรมของ กล้อง
        ...
        'rgbd_cameras': 2,              # จำนวนกล้อง rgbd
        ...
        'Vis/MinDepth': '0.6',          # ระยะกล้องใกล้สุด
        'Vis/MaxDepth': '6.0',          # ระยะกล้องไกลสุด
        'Vis/MinInliers': '20',         # ความสอดคล้องของคุณลักษณะขั้นต่ำในการคำนวณ/ยอมรับการแปลง
        'Vis/EstimationType': '0',
        'ORB/EdgeThreshold': '19',      # นี่คือขนาดของเส้นขอบที่ไม่พบฟีเจอร์ใดๆ ซึ่งควรจะตรงกับพารามิเตอร์ patchSize โดยประมาณ
        'ORB/FirstLevel': '0',          # ควรเป็น 0 ในการใช้งานปัจจุบัน
        'ORB/Gpu': 'false',             # การใช้ gpu
        'ORB/NLevels': '3',             # จำนวนระดับพีระมิด ระดับที่เล็กที่สุดจะมีขนาดเชิงเส้นเท่ากับ input_image_linear_size/pow(scaleFactor, nlevels)
        'ORB/PatchSize': '31',          # ขนาดของแพตช์ที่ใช้โดยตัวระบุ BRIEF แบบวางแนว แน่นอนว่าในเลเยอร์พีระมิดขนาดเล็ก พื้นที่ภาพที่รับรู้ซึ่งครอบคลุมโดยฟีเจอร์จะมีขนาดใหญ่กว่า
    ...
    rgbd_odometry = Node(
        ...
        parameters=[{
            ...
            'Odom/KalmanMeasurementNoise': '0.01',  # ค่าความแปรปรวนร่วมของเสียงในกระบวนการ
            'Odom/KalmanProcessNoise': '0.001',     # ค่าความแปรปรวนร่วมในการวัดกระบวนการ
    ...
    rtabmap_slam = Node(
        ...
        output='screen',
        parameters=[{
            'subscribe_scan_cloud': True,           # ใช้งาน scan cloud Lidar
            'odom_sensor_sync': False,              # odom ของทุกเซนเซอร์ sync กัน
            # RTAB-Map's internal parameters are strings:
            ...
            'RGBD/LinearUpdate': '0.1',            # การเคลื่อนที่เชิงเส้นขั้นต่ำเพื่ออัปเดตแผนที่ การซ้อมจะทำก่อนหน้านี้ ดังนั้นน้ำหนักจึงยังคงได้รับการอัปเดต
            ...
            'Mem/NotLinkedNodesKept': 'false',      # ไม่เก็บโหนดที่เชื่อมโยงไว้ในฐานข้อมูล (โหนดที่ซ้อมและโหนดที่ถูกลบ)
            ...
            'Icp/RangeMin': '0.25',                 # ระยะ Lidar ใกล้สุด
            'Icp/RangeMax': '100.0',                # ระยะ Lidar ไกลสุด
            'Icp/VoxelSize': '0',                   # ขนาดวอกเซลการสุ่มตัวอย่างแบบสม่ำเสมอ (0=ปิดใช้งาน)
            'Icp/PointToPlaneK': '30',              # จำนวนเพื่อนบ้านที่จะคำนวณค่าปกติสำหรับจุดต่อระนาบ
            ...
            'Icp/Epsilon': '0.001',                 # ตั้งค่า epsilon ของการแปลง (ค่าความแตกต่างสูงสุดที่อนุญาตระหว่างการแปลงสองครั้งติดต่อกัน) เพื่อให้พิจารณาว่าการเพิ่มประสิทธิภาพนั้นบรรจบกับโซลูชันสุดท้ายแล้ว
            'Icp/MaxTranslation': '1.5',            # การยอมรับการแก้ไขการแปล ICP สูงสุด (ม.)
            'Icp/MaxCorrespondenceDistance': '0.1', # ระยะทางสูงสุดสำหรับการโต้ตอบจุด
    ...
    lidar_tf = Node(
        ...
        arguments=[
            ...
            "0.05", # translation_z
            "1.5707",  # rotation_yaw
            "0.7854",# rotation_pitch
            "0.0", # rotation_roll
            "D456_1_link",  # parant D456_1_link
            "lidar_link",   # child
        ]
    )
